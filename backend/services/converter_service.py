...
class ConverterService:
    ...
    def _build_ffmpeg_args(
        ...
        # --- Video stream ---
        args += ["-map", "0:V:0?"]  # First video stream (excluding attached pictures)
        ...
        # --- Audio: map selected tracks, re-encode to AAC or Copy ---
        if job.keep_all_audio or job.audio_tracks is None:
            args += ["-map", "0:a?"]
        else:
            for idx in job.audio_tracks:
                args += ["-map", f"0:{idx}"]
        ...
        # --- Subtitles: map selected tracks, stream copy ---
        if job.keep_all_subtitles or job.subtitle_tracks is None:
            args += ["-map", "0:s?"]
        else:
            for idx in job.subtitle_tracks:
                args += ["-map", f"0:{idx}"]
